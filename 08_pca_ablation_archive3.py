...
def main():
    ...
    for dataset_name, path in DATASETS.items():
        if not path.exists():
            raise FileNotFoundError(f'{path} not found. Run 02_split_dataset_archive3.py / 05_fusion_archive3.py first.')
        df = pd.read_csv(path)
    ...
    if not with_pca_path.exists():
        print('\nWith-PCA results not found; run 07_compare_results_pca_archive3.py first to get a side-by-side comparison.')
        return
