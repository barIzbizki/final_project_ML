...
def subsample_for_svm(X_train: pd.DataFrame, y_train: pd.Series):
    # See SVM_MAX_TRAIN_ROWS in config.py for why only this model is subsampled.
...
def evaluate_dataset_with_pca(
    ...
    for model_name, model in create_models().items():
        ...
        X_fit, y_fit = (
            subsample_for_svm(X_train, y_train)
            if model_name == "SVM"
...
# ============================================================
# השוואה מול תוצאות ללא PCA
# ============================================================

def compare_with_existing_without_pca_results(
    with_pca_df: pd.DataFrame,
) -> None:
    without_path = (
        OUTPUT_DIR
        / "all_results_without_pca.csv"
    )

    if not without_path.exists():
        print(
            "\nWithout-PCA result file was not found. "
            "The PCA experiment was completed, "
            "but no comparison table was created."
        )
        return
    ...
    pca_selected = with_pca_df[
        ["Dataset", "Model"] + metrics
    ].copy()

    without_selected = without_df[
        ["Dataset", "Model"] + metrics
    ].copy()
    ...
    comparison_df.to_csv(
        OUTPUT_DIR
        / "pca_vs_without_pca_comparison.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def main() -> None:
    ...
    compare_with_existing_without_pca_results(
        results_df
    )
